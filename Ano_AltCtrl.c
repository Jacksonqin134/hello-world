...
// 高度串级PID控制: 1.外环高度PID控制 2.内环高度速度PID控制
...
{
	alt_arg_1.kp = Ano_Parame.set.pid_alt_1level[KP];		//Ano_Parame.set.pid_alt_1level[KP] = (float) ParValList[PAR_PID_7_P] / 1000;
	alt_arg_1.ki = Ano_Parame.set.pid_alt_1level[KI];		//Ano_Parame.set.pid_alt_1level[KI] = (float) ParValList[PAR_PID_7_I] / 1000;
...
void Alt_1level_Ctrl(float dT_s)
{
	...
	// r(k)
	loc_ctrl_1.exp[Z] = 0.6f *fs.alt_ctrl_speed_set + alt_val_2.out;//速度前馈0.6f，直接给速度，alt_val_2.out:高度环PID输出
														//fs.alt_ctrl_speed_set = fs.speed_set_h[Z] + auto_taking_off_speed;
	...
	if(flag.taking_off == 1)
	{
		LPF_1_(1.0f,dT_s,THR_START *10,err_i_comp);//((err_i_comp) += ( 1 / ( 1 + 1 / ( (1) *6.28f *(dT_s) ) ) ) *( (THR_START) - (err_i_comp) ))			
	}
	else
	{
		err_i_comp = 0;
	...
	loc_ctrl_1.out[Z] = out_en *(alt_val_1.out + err_i_comp);				// err_i_comp = THR_START *10;	#define THR_START  35  //油门起调量百分比 %
	
	loc_ctrl_1.out[Z] = LIMIT(loc_ctrl_1.out[Z],0,MAX_THR_SET *10);			// #define MAX_THR_SET    85  //最大油门百分比 %
	
	mc.ct_val_thr = loc_ctrl_1.out[Z];		// enum Z=2
}
