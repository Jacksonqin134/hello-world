...
{
	unsigned char CK_A = 0, CK_B = 0;			// CK_A CK_B 动态相对定位
	unsigned short  i;
...
void Drv_GpsPin_Init(void)
{
	...
	USART_InitStructure.USART_Mode              =   USART_Mode_Rx | USART_Mode_Tx;		// 模式设置为发送模式或者接收模式
...
{  
	...
	{   
		USART_ClearITPendingBit(GPS_UART,USART_IT_RXNE);  		// 清除中断标志位
