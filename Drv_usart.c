...
void Usart2_Init ( u32 br_num )
{
    ...
    //使能USART2接收中断
    USART_ITConfig ( USART2, USART_IT_RXNE, ENABLE );			// 中断
    //使能USART2
...
void Usart2_IRQ ( void )
{
    ...
    //接收中断
    if ( USART_GetITStatus ( USART2, USART_IT_RXNE ) )		// 判断接受数据是否为空
    {
        USART_ClearITPendingBit ( USART2, USART_IT_RXNE ); //清除中断标志

				com_data = USART2->DR;			// 不为空就从DR寄存器读取数据
    ...
    if ( USART_GetITStatus ( USART2, USART_IT_TXE ) )
    {
				...
				USART2->DR = TxBuffer[TxCounter++]; 		// 向DR寄存器写入要发送的数据，本质是存储在发送缓冲区
        if ( TxCounter == count )
        {
...
void Usart3_Init ( u32 br_num )
{
    ...
    //使能USART3接收中断
    USART_ITConfig ( USART3, USART_IT_RXNE, ENABLE );				// ITConfig 标志很重要，
    //使能USART2
